...
public final class World {
    ...
    public static void setupBasicLogisticsChain(World world) {
        ...
        Wire wire1 = Entities.WIRE_ENTITY_TYPE.create(world, new Location(150, 100));
        Wire wire2 = Entities.WIRE_ENTITY_TYPE.create(world, new Location(250, 100));
        Trash trash = Entities.TRASH_ENTITY_TYPE.create(world, new Location(350, 250));

        world.addEntity(spawner);

        world.addEntity(wire1);
        world.addEntity(wire2);
    ...
    public void render(RenderContext gameRenderContext) {
        // 1. COLLECT PHASE
        // Every entity (Spawner, Wire, etc.) adds their tasks to the context
        for (Entity e : entities) {
